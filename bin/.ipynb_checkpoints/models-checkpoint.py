...
class models:
    def get_model(self, config):
        if config.model == 'baseline':
            model = self.baseline(config)
        elif config.model == 'baseline_attention':
            model = self.baseline_attention(config)
        elif config.model == 'baseline_attention_fc':
            model = self.baseline_attetion_fc(config)
        elif config.model == 'baseline_preconv_attention':
            model = self.baseline_preconv_attetion(config)
        elif config.model == 'dense_attention':
            model = self.dense_attention(config)
        return model
    ...
    def baseline(self, config):
        ...
        x = Bidirectional(CuDNNGRU(config.cell_size_l1, return_sequences=True))(emb)
        x = Bidirectional(CuDNNGRU(config.cell_size_l2, return_sequences=False))(x)
        outp = Dense(2, activation='softmax')(x)

        model = Model(inputs=inp, outputs=outp)
        model.compile(loss='categorical_crossentropy', optimizer=Adam(lr=0.001, decay=0.0001), metrics=['categorical_crossentropy', 'accuracy'])
        return model

    def baseline_attetion(self, config):
        inp = Input(shape=(config.strmaxlen, ), name='input')
        emb = Embedding(config.max_features, config.max_features,embeddings_initializer='identity', trainable = True)(inp)
        emb = SpatialDropout1D(config.prob_dropout)(emb)
        x = Bidirectional(CuDNNGRU(config.cell_size_l1, return_sequences=True))(emb)
        x = Bidirectional(CuDNNGRU(config.cell_size_l2, return_sequences=True))(x)
        x = Attention(config.strmaxlen)(x)
        outp = Dense(2, activation='softmax')(x)
    ...
    def baseline_attetion_fc(self, config):
        inp = Input(shape=(config.strmaxlen, ), name='input')
        emb = Embedding(config.max_features, config.max_features,embeddings_initializer='identity', trainable = True)(inp)
        emb = SpatialDropout1D(config.prob_dropout)(emb)
        x = Bidirectional(CuDNNGRU(config.cell_size_l1, return_sequences=True))(emb)
    ...
    def baseline_preconv_attetion(self, config):
        ...
        emb = Embedding(config.max_features, config.max_features,embeddings_initializer='identity', trainable = True)(inp)
        emb = SpatialDropout1D(config.prob_dropout)(emb)
        x = Conv1D(config.filter_size, kernel_size = config.kernel_size, strides=config.strides, padding = "valid", kernel_initializer = "he_uniform")(emb)
        x = Bidirectional(CuDNNGRU(config.cell_size_l1, return_sequences=True))(x)
        x = Bidirectional(CuDNNGRU(config.cell_size_l2, return_sequences=True))(x)
    ...
    def dense_attention(self, config):
        ...
        #### 
        l1_G = Bidirectional(CuDNNGRU(config.cell_size_l1, return_sequences=True))(emb)
        l2_GG = Bidirectional(CuDNNGRU(config.cell_size_l2, return_sequences=True))(l1_G)
        l3_GGC = Conv1D(config.filter_size, kernel_size = config.kernel_size, strides=config.strides, padding = "valid", kernel_initializer = "he_uniform")(l2_GG)
...
#       ==================================================================================================
        model = Model(inputs=inp, outputs=outp)
        model.compile(loss='categorical_crossentropy', optimizer=Adam(lr=0.001, decay=0.0005), metrics=['categorical_crossentropy', 'accuracy'])
